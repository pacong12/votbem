...
class Kandidatdpm extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->library('ion_auth', 'form_validation', 'session', 'form_helper');
        $this->load->model('Kandidatdpm_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'kandidatdpm/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'kandidatdpm/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'kandidatdpm/index.html';
            $config['first_url'] = base_url() . 'kandidatdpm/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Kandidatdpm_model->total_rows($q);
        $kandidat = $this->Kandidatdpm_model->get_limit_data($config['per_page'], $start, $q);
    ...
    public function read($id)
    {
        ...
        if ($row) {
            ...
            $this->load->view('back/kandidatdpm/kandidat_read', $data);
        } else {
    ...
    public function create()
    {
        ...
        $this->load->view('back/kandidatdpm/kandidat_form', $data);
    }

    public function create_action()
    {
        $config['upload_path'] = './assets/uploads/kandidatdpm';
    ...
    public function delete($id)
    {
        ...
        if ($row) {
            $this->Kandidatdpm_model->delete($id);
            unlink('./assets/uploads/kandidatdpm/' . $image);
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-success alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
    ...
    public function deactivate($id)
    {
        ...
        $this->Kandidatdpm_model->update($id, $data);
        $this->session->set_flashdata(
            'message',
            '<div class="alert alert-success alert-dismissible">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
